...
func TestMapPack(t *testing.T) {
	...
	for i, item := range testData {
		...
		mr, cs, n, err := currency.UnpackMap(buffer, true)
		if nil != err {
			t.Fatalf("%d: unpack error: %s", i, err)
		}

		if len(buffer) != n {
			t.Fatalf("%d: unpack counsumed: %d of: %d", i, n, len(buffer))
		}
...
func TestMapUnpackBuffer(t *testing.T) {
	...
	testData := []struct {
		...
		{
			b: []byte{
				0x02, 0x02, 0x22, 0x6d, 0x6d, 0x43, 0x4b, 0x5a,
				0x53, 0x37, 0x74, 0x6f, 0x45, 0x36, 0x39, 0x51,
				0x67, 0x58, 0x4e, 0x73, 0x31, 0x4a, 0x5a, 0x63,
				0x6a, 0x57, 0x36, 0x4c, 0x46, 0x6a, 0x38, 0x4c,
				0x66, 0x55, 0x62, 0x7a, 0x36, 0x01, 0x22, 0x6d,
		...
		{
			b: []byte{
				0x02, 0x01, 0x22, 0x6d, 0x69, 0x70, 0x63, 0x42,
				0x62, 0x46, 0x67, 0x39, 0x67, 0x4d, 0x69, 0x43,
				0x68, 0x38, 0x31, 0x4b, 0x6a, 0x38, 0x74, 0x71,
				0x71, 0x64, 0x67, 0x6f, 0x5a, 0x75, 0x62, 0x31,
				0x5a, 0x4a, 0x52, 0x66, 0x6e, 0x02, 0x22, 0x6d,
				0x6d, 0x43, 0x4b, 0x5a, 0x53, 0x37, 0x74, 0x6f,
				0x45, 0x36, 0x39, 0x51, 0x67, 0x58, 0x4e, 0x73,
				0x31, 0x4a, 0x5a, 0x63, 0x6a, 0x57, 0x36, 0x4c,
				0x46, 0x6a, 0x38, 0x4c, 0x66, 0x55, 0x62, 0x7a,
				0x36,
			},
			m: currency.Map{
				currency.Litecoin: "mmCKZS7toE69QgXNs1JZcjW6LFj8LfUbz6",
	...
	for i, item := range testData {
		...
		mr, cs, n, err := currency.UnpackMap(item.b, true)
		if nil != err {
			t.Fatalf("%d: unpack error: %s", i, err)
		}

		if len(item.b) != n {
			t.Fatalf("%d: unpack counsumed: %d of: %d", i, n, len(item.b))
		}
		for c, a := range item.m {
			if a != mr[c] {
				t.Fatalf("%d: unpack actual: %+v  expected: %+v", i, mr, item.m)
...
func TestMapUnpackInvalid(t *testing.T) {
	...
	testData := []struct {
		...
		{
			b: []byte{
				0x02, 0x00, 0x22, 0x6d, 0x6d, 0x43, 0x4b, 0x5a,
				0x53, 0x37, 0x74, 0x6f, 0x45, 0x36, 0x39, 0x51,
				0x67, 0x58, 0x4e, 0x73, 0x31, 0x4a, 0x5a, 0x63,
				0x6a, 0x57, 0x36, 0x4c, 0x46, 0x6a, 0x38, 0x4c,
				0x66, 0x55, 0x62, 0x7a, 0x36, 0x01, 0x22, 0x6d,
		...
		{
			b: []byte{
				0x02, 0xff, 0xff, 0x7f, 0x22, 0x6d, 0x6d, 0x43,
				0x4b, 0x5a, 0x53, 0x37, 0x74, 0x6f, 0x45, 0x36,
				0x39, 0x51, 0x67, 0x58, 0x4e, 0x73, 0x31, 0x4a,
				0x5a, 0x63, 0x6a, 0x57, 0x36, 0x4c, 0x46, 0x6a,
				0x38, 0x4c, 0x66, 0x55, 0x62, 0x7a, 0x36, 0x01,
				0x22, 0x6d, 0x69, 0x70, 0x63, 0x42, 0x62, 0x46,
				0x67, 0x39, 0x67, 0x4d, 0x69, 0x43, 0x68, 0x38,
				0x31, 0x4b, 0x6a, 0x38, 0x74, 0x71, 0x71, 0x64,
				0x67, 0x6f, 0x5a, 0x75, 0x62, 0x31, 0x5a, 0x4a,
				0x52, 0x66, 0x6e,
			},
			err: fault.ErrInvalidCurrency,
		},
		...
		{
			b: []byte{
				0xff, 0xff, 0x02, 0x02, 0x22, 0x6d, 0x6d, 0x43,
				0x4b, 0x5a, 0x53, 0x37, 0x74, 0x6f, 0x45, 0x36,
				0x39, 0x51, 0x67, 0x58, 0x4e, 0x73, 0x31, 0x4a,
				0x5a, 0x63, 0x6a, 0x57, 0x36, 0x4c, 0x46, 0x6a,
				0x38, 0x4c, 0x66, 0x55, 0x62, 0x36, 0x71, 0x01,
				0x22, 0x6d, 0x69, 0x70, 0x63, 0x42, 0x62, 0x46,
				0x67, 0x39, 0x67, 0x4d, 0x69, 0x43, 0x68, 0x38,
				0x31, 0x4b, 0x6a, 0x38, 0x74, 0x71, 0x71, 0x64,
				0x67, 0x6f, 0x5a, 0x75, 0x62, 0x31, 0x5a, 0x4a,
	...
	for i, item := range testData {
		...
		_, _, _, err := currency.UnpackMap(item.b, true)
		if item.err != err {
			t.Fatalf("%d: error: %s  expected: %s", i, err, item.err)
		}
	}
